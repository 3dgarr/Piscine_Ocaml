...
let newDeck () : t =
  let deck = Card.all in
  let deck_with_keys = List.map (fun card -> (Random.bits (), card)) deck in
  let shuffled_with_keys = List.sort compare deck_with_keys in
  let shuffled_deck = List.map snd shuffled_with_keys in
  { cards = shuffled_deck }

let toStringList (deck: t) : string list =
  List.map Card.toString deck.cards

let toStringListVerbose (deck: t) : string list =
  List.map Card.toStringVerbose deck.cards

let drawCard (deck: t) : Card.t * t =
  match deck.cards with
  | [] -> failwith "drawCard: deck is empty"
  | card :: rest -> (card, { cards = rest })
